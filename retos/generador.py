...
class GeneradorRetos:
    ...
    def __init__(self, diccionario, analizador, grafo, generador_oraciones, motor_srs):
        ...
        if not self.grafo.construido:
            print("Grafo no construido, construyendo ahora...")
            self.grafo.construir()
        
    def generar_sesion_practica(self, nivel_usuario: int = 50, 
        num_retos: int = 5, tipos_permitidos: List[str] = None) -> List:
        """
        ...
        if len(palabras_pendientes) < num_retos:
            print(f"DEBUG: Pocas palabras en SRS, obteniendo del grafo...")
            ...
            for categoria in categorias:
                try:
                    ...
                    print(f"DEBUG: Error obteniendo categoría {categoria}: {e}")
            
            if not palabras_grafo:
                print("DEBUG: No se pudieron obtener palabras del grafo, usando lista básica")
        ...
        if len(palabras_a_practicar) < num_retos:
            print(f" DEBUG: Solo se generarán {len(palabras_a_practicar)} retos (menos de los {num_retos} solicitados)")
        ...
        for i, palabra in enumerate(palabras_a_practicar):
            ...
            try:
                reto = self.crear_reto(tipo=tipo_reto, palabra=palabra, nivel_usuario=nivel_usuario)
                if reto:
                    retos.append(reto)
                    print(f"Reto creado exitosamente")
                else:
                    print(f"No se pudo crear el reto")
            except Exception as e:
    ...
    def _seleccionar_tipo_reto(self, nivel_usuario: int, posicion_sesion: int,
        total_sesion: int, tipos_permitidos: List[str] = None) -> str:
        """
    ...
    def crear_reto(self, tipo: str, palabra: str, nivel_usuario: int = 50,
                  **kwargs) -> Optional[Any]:
        """
        ...
        if not info_palabra:
            print(f" Palabra '{palabra}' no encontrada en diccionario, intentando con otra...")
        ...
        try:
            if tipo == 'tarjetas':
                return RetoTarjetas(palabra_objetivo=palabra, diccionario=self.diccionario,
                    nivel_dificultad=nivel_str, tipo=kwargs.get('tipo_tarjeta', 'traduccion'),
                    num_opciones=kwargs.get('num_opciones', 4))
            elif tipo == 'tarjetas_inverso':
                return RetoTarjetasInverso(palabra_objetivo=palabra, diccionario=self.diccionario,
                    nivel_dificultad=nivel_str, tipo=kwargs.get('tipo_tarjeta', 'traduccion'),
                    num_opciones=kwargs.get('num_opciones', 4))
            elif tipo == 'formar_palabras':
                letras_extra = 0 if nivel_usuario < 40 else 1 if nivel_usuario < 70 else 2
                return RetoFormarPalabras(palabra_objetivo=palabra, diccionario=self.diccionario,
                        analizador=self.analizador, nivel_dificultad=nivel_str, con_pista=nivel_usuario < 60,
                    letras_extra=letras_extra)
            elif tipo == 'completar_oracion':
                return RetoCompletarOracion(palabra_objetivo=palabra, generador=self.generador_oraciones,
                    diccionario=self.diccionario, analizador=self.analizador, nivel_dificultad=nivel_str,
                    con_opciones=nivel_usuario < 50)
            elif tipo == 'ordenar_oracion':
                return RetoOrdenarOracion(palabra_objetivo=palabra, generador=self.generador_oraciones,
                    analizador=self.analizador, nivel_dificultad=nivel_str)
            elif tipo == 'traducir_oracion':
                return RetoTraducirOracion(palabra_objetivo=palabra, generador=self.generador_oraciones,
                    diccionario=self.diccionario, analizador=self.analizador,
                    nivel_dificultad=nivel_str, ingles_a_espanol=kwargs.get('ingles_a_espanol', True))
                ...
                return RetoFormarPalabrasMultiple(palabras_objetivo=palabras_objetivo, diccionario=self.diccionario,
                    analizador=self.analizador, nivel_dificultad="avanzado")
            else:
    ...
    def crear_reto_personalizado(self, preferencias_usuario: Dict[str, Any]) -> Optional[Any]:
        ...
        tipo = self._seleccionar_tipo_reto(nivel_usuario=nivel, posicion_sesion=0,
            total_sesion=1, tipos_permitidos=tipos_favoritos)
